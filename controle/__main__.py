...
for index, usuario in enumerate(usuarios, start=1):
    ...
    espaco_utilizado_megabytes = (
        f'{converter_bytes_para_megabytes(usuario.espaco_utilizado):.2f} MB'
    )
...
with open('arquivos/relatorio.txt', mode='w', encoding='utf-8') as file:
    linhasParaEscrever = [
        ...
        *[STRING_FORMATACAO.format(*l) for l in linhas],
        ...
        'Espaço médio ocupado: {:.2f} MB'.format(
            converter_bytes_para_megabytes(total) / len(usuarios)
        ),
    ]
